---
name: prototype
description: Prototype an idea in Magic Patterns (magicpatterns.com) starting from your local UI. Use when the user gives a prompt for something they want to prototype or explore and wants an iterable Magic Patterns design grounded in their existing code. Seeds a design by mirroring the relevant local UI, then prompts Magic Patterns to design the new idea (direct file writes only for precise tweaks afterward), and returns the editor link. Requires the Magic Patterns MCP server.
---

# Prototype an idea in Magic Patterns

Use this skill when the user has an idea they want to **prototype** and wants it grounded in the UI they already have. It seeds a Magic Patterns design from the relevant local UI, then prompts Magic Patterns to design the idea, handing back the editor link.

It composes two mechanisms, used **in order, for different jobs** — not interchangeably:

- **Seed (deterministic):** mirror the relevant *existing* local UI into a blank design with `write_artifact_files` (the `upload-to-magic-patterns` flow). This recreates what already exists — it does NOT build the new idea.
...
Magic Patterns renders **visual prototypes**, not production apps — local code must be ported to its constraints (see the `upload-to-magic-patterns` skill).
...
The two mechanisms are sequential and do different jobs: **seed** mirrors existing UI deterministically, then **prompt** lets Magic Patterns design the new idea. Do them in this order.
...
Run the `upload-to-magic-patterns` flow to mirror the relevant local UI into a fresh design (follow that skill for the porting constraints — mock data, Tailwind import order, `lucide-react`, named exports, must-compile):

1. `create_design` with no prompt → blank design; returns `editorId`, active `artifactId`, and the `editorUrl`. Keep them.
2. `get_design_status(editorId)` → confirm the active `artifactId` and `isGenerating` is false.
3. `write_artifact_files(artifactId, files)` → write the ported local files directly.
4. `publish_artifact(artifactId, editorId)` → compile + activate.
...
**Seed fidelity matters most here.** The generated feature inherits the seed's look and layout, so an approximate seed produces an approximate (often ugly) result. Reproduce the existing UI faithfully — and prefer importing the source's real component library, which MP installs from `package.json` (e.g. Cubed → `@radix-ui/themes` + its theme CSS + a `<Theme>` wrapper), rather than stubbing or rebuilding it in approximate Tailwind. A faithful, well-sized seed is what keeps the later `send_prompt` result from clashing or overflowing. See `upload-to-magic-patterns` Step 2 ("Reproduce the source faithfully").

### Step 3 — Hand back the editor link (checkpoint)

Give the user the `editorUrl` so they can open the starting point in their browser before spending generation credits. Tell them they may need to log in.
...
3. Generation runs in the background. Hand the user the `editorUrl` and tell them to open it in their browser. Poll `get_design_status` at most once every 60 seconds only if you need to programmatically inspect the result afterward (e.g. `read_artifact_files`).
...
After each meaningful change, return the `editorUrl` so the user can view and keep iterating in Magic Patterns.

Offer to keep iterating, or to pull the result back into the codebase with the `integrate-magic-patterns-design` skill.
...
- `upload-to-magic-patterns` — the seed step: push local UI into a design by writing files directly.
